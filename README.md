...
顶部菜单「🎤 面试模拟」进入。上传简历（PDF/TXT/Markdown 自动提取，或粘贴正文，或填**简历链接**——
后端会抓取该网页/PDF 并提取文字，面试官才能真正读到简历内容）、
...
全程支持**语音对话**：面试官回答边输出文字边朗读（TTS，默认 `gpt-4o-mini-tts` 男声 `onyx`，拟人），
你也可以点麦克风**语音作答**——默认走 **Whisper**（`/v1/audio/transcriptions`）转写，更准且各浏览器通用，未配置时回退浏览器原生识别。
打字时 **Enter 发送、Alt+Enter 换行**（中文输入法组字中的回车不会误发）。
