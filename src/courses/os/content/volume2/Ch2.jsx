    ...
    <>
      ...
      <Summary
        points={[
          ...
          'FIFO 实现最简单但会出现 Belady 异常（加页框反而缺页更多），因为它和未来访问毫无关系。',
          'Clock（二次机会）是 LRU 的高效近似，用访问位避免维护精确顺序；LFU 按访问次数淘汰。',
          'LRU 的经典实现是双向链表+哈希表做到 O(1)；Redis 用随机采样的近似 LRU 来省内存。',
          '工作集装不进物理页框就会抖动（thrashing），CPU 忙于换页，应按工作集分配页框或挂起进程。',
        ]}
      />
    </>
  )
