...
const electionStateCode = `// 选举期间每个节点的内部状态（peerState）
LOOKING     // 正在找 Leader / 投票中（崩溃恢复阶段就处于这个状态）
FOLLOWING   // 已确认自己是 Follower，跟随某个 Leader
LEADING     // 已确认自己是 Leader，开始广播
OBSERVING   // Observer 状态，只同步数据不参与投票

// 一条投票的内容：(推选的 myid, 该候选的 zxid, 该候选的 epoch)
// 收到别人投票后按 epoch > zxid > myid 比较，谁优投谁`

const observerConfig = `# 把某台机器配成 Observer：在它的 zoo.cfg 里加一行
peerType=observer
# 并在所有机器的成员列表里给它标 :observer
server.1=zk1:2888:3888
server.2=zk2:2888:3888
server.3=zk3:2888:3888
server.4=zk4:2888:3888:observer   # 第 4 台是 Observer，不参与过半计算

# 价值：读 QPS 不够时，加 Observer 横向扩读，
# 又不会因为成员变多而拖慢写（写只需投票节点过半，Observer 不投票）`
...
export default function Ch1() {
  return (
    <>
      ...
      <p>
        严格来说 ZAB 一个完整周期分四个阶段：<strong>选举（Election）→ 发现（Discovery）→ 同步（Synchronization）→ 广播（Broadcast）</strong>。
        前三个都属于崩溃恢复。发现阶段，准 Leader 收集 Follower 各自的最新 zxid，确定一个新的 epoch；
        同步阶段，准 Leader 把自己的事务历史推给 Follower，让大家数据对齐到同一起点；只有过半 Follower 完成同步，
        才正式升为 Leader 进入广播。理解这四阶段能让你回答「选完主为什么不能立刻写」——因为还要先把数据对齐。
      </p>
      ...
      <p>
        Observer 是个很实用又常被忽略的角色。ZooKeeper 写性能受限于「过半投票」，集群越大写越慢；但读可以无限扩。
        于是当你需要支撑海量读、或者要跨机房部署（让远端机房的客户端就近读，又不让跨机房的高延迟拖慢投票），
        就用 Observer：它接收 Leader 的广播、对外提供读，但不参与投票。
      </p>
      <CodeBlock lang="properties" title="配置一台 Observer" code={observerConfig} />
      ...
      <p>
        为什么最后用 myid 兜底？因为 myid 是配置里人为指定、全局唯一的，能保证任何僵局都有唯一确定的胜者，
        避免「平票卡死」。这也解释了一个常见现象：全新集群首次启动、所有节点 zxid 都是 0 时，
        往往是 myid 最大的那台当选 Leader。生产上若想让某台高配机器优先当主，把它的 myid 设大一点即可。
      </p>
      <CodeBlock lang="java" title="选举状态与投票内容" code={electionStateCode} />
      ...
      <Example title="Leader 宕机，秒级选出新主">
        ...
        <p>
          一个关键边界：在 zk2、zk3 重新选举并完成同步之前，集群<strong>对外不可写</strong>（这正是 CP 的体现，
          宁可短暂不可用也不返回不一致数据）。但只剩两台、还没选出主时，读请求由谁服务？此刻处于 LOOKING 的节点
          会拒绝服务；只有选出 Leader、过半同步完成，才恢复读写。所以「Leader 宕机到恢复服务」的真实耗时，
          = 心跳判死时间 + 选举时间 + 数据同步时间，不是只有选举那一下。
        </p>
      </Example>
      ...
      <KeyIdea title="为什么 zxid 最大的当 Leader">
        ...
        <p>
          这里有个微妙之处：一个写只要被过半节点写入了事务日志，它就「可能已对客户端返回成功」，必须不丢。
          而过半集合与「拥有最新 zxid 的存活集合」必然有交集（鸽巢原理：两个过半集合一定相交），
          所以新一轮选举里一定有节点持有那条已提交的最新事务，zxid 最大者当选就把它带了进来。
          同步阶段还会处理一类特殊情况：某条提案 Leader 发了但没等到过半 ack 就崩了——这种「悬而未决」的事务，
          新 Leader 会按规则统一丢弃或补提交，保证全集群对每条事务的最终结论一致。
        </p>
      </KeyIdea>
      ...
      <p>
        实现上它还有个「收敛确认」窗口：某候选拿到过半票后，并不立即拍板，而是再等一小段时间
        （<code>finalizeWait</code>，默认 200ms）看有没有更优投票涌进来，没有才确定结果。这避免了「刚定下来又被
        更优候选推翻」的反复。选举走的是独立的<strong>选举端口（3888）</strong>，数据同步走<strong>2888</strong>，
        两者分开是为了选举流量不被正常同步流量干扰——这也是为什么 zoo.cfg 里每台要配两个端口。
      </p>
      ...
      <Callout variant="warn" title="集群为什么推荐奇数台">
        ...
        <p>
          顺带说脑裂：假设 5 台分成 2+3 两个分区，只有「3 台」那边能凑齐过半、继续工作并选主，「2 台」那边因为
          永远凑不齐过半而拒绝服务——所以 ZooKeeper <strong>天生不会脑裂出两个 Leader</strong>。这正是过半机制最珍贵的副产品。
          想要更省机器又保容错，可以了解 3.5+ 的 <strong>动态重配置（reconfig）</strong>和带权重/层级的仲裁配置。
        </p>
      </Callout>
      ...
      <p>
        高频追问：<strong>「ZAB 和 Raft 什么关系？」</strong>——思路高度相似（强 Leader、过半提交、用任期号防旧主），
        ZAB 早于 Raft，Raft 可看作把这类协议讲得更易懂的工程化版本；最大差异是 ZAB 为「主备广播」而生、
        Raft 为「复制状态机」而生，且 ZAB 的恢复阶段处理悬而未决事务的逻辑更复杂。
        <strong>「ZK 会脑裂吗？」</strong>——不会出现双 Leader，过半机制保证少数派分区无法选主。
        <strong>「Observer 影响选举吗？」</strong>——不影响，它不投票不计入过半。
      </p>
      ...
      <Practice title="搭一个三节点集群">
        ...
        <p>
          进阶：再 kill 掉一台，集群只剩 1 台，此时凑不齐过半（需要 2），你会发现剩下这台进入 LOOKING、
          整个集群<strong>拒绝读写</strong>——亲手验证 CP 与过半机制。把刚才那台拉起来恢复到 2 台，服务立刻回来。
        </p>
      </Practice>
      ...
      <Summary
        points={[
          ...
          '完整周期分选举→发现→同步→广播四阶段，选完主还要过半同步对齐数据才能对外服务。',
          '集群三种角色：Leader 处理写并广播，Follower 投票/读/转发写，Observer 只读不投票，用来横向扩读和跨机房。',
          '选举比较顺序是 epoch > zxid > myid，zxid 越大代表数据越新，必须选数据最新者当 Leader 才不丢数据。',
          '默认算法 FastLeaderElection：先投自己再广播，遇到更优候选就改投，过半票即收敛，故称快速选举。',
          '过半机制天生防脑裂：网络分区时只有过半的一侧能选主工作，少数派拒绝服务，不会出现双 Leader。',
          '集群依赖过半才能工作，推荐 3/5/7 奇数台，偶数台浪费机器且分区时更易整体不可用。',
          'ZAB 与 Raft 思路相通（强 Leader、过半提交、任期号防旧主），ZAB 为主备广播而生、恢复逻辑更复杂。',
        ]}
      />
    </>
  )
