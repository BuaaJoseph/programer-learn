...
export default function Ch4() {
  return (
    <>
      ...
      <p>
        举个例子体会「不灵活」：当模型几乎笃定下一个词是「北京」（概率 0.99）时，top-k=40 还硬要保留另外 39 个概率加起来才 0.01 的词，
        给它们一个本不该有的被抽中机会；反过来当模型在写诗、几百个词都合理时，k=40 又粗暴砍掉了大量本可入选的好词。
        症结就在于「该留几个」本该随分布形状变，而 k 是个死数。这正是 top-p 用「累计概率」取代「固定数量」的动机。
      </p>
      ...
      <h3>seed：在随机里求可复现</h3>
      <p>
        采样本质是「按概率掷骰子」，那同一个 prompt 每次结果都不同吗？只要温度大于 0，默认是的。
        部分 API 提供 <code>seed</code> 参数：固定随机数种子后，「掷骰子」的随机序列就被钉死，相同输入 + 相同 seed + 相同模型版本，
        理论上能拿到一致输出。这对调试、写测试、做可复现实验很关键——你不希望每跑一次结果都变，没法定位问题。
      </p>
      <p>
        但要泼盆冷水：seed 只控制采样这一步的随机，<strong>挡不住下面那条警告里讲的浮点、批处理、MoE 抖动</strong>。
        所以线上即便固定了 seed，仍可能偶有差异。把 seed 当成「大幅提高复现概率」的工具，而不是「保证逐字一致」的开关。
      </p>
