...
export default function Ch5() {
  return (
    <>
      ...
      <p>
        为什么一个「加法」就能解决梯度消失？看反向传播：<code>x = x + 子层(x)</code> 对输入求导，会得到「1 + 子层的导数」。
        那个 <strong>+1</strong> 是关键——它保证了梯度往回传时，总有一条「直通车」原样穿过这一层，不会被层层连乘的小数字（小于 1）
        逐渐乘没。没有残差时，梯度要穿过几十层连乘，很容易指数级衰减到 0（深层学不动）或爆炸；有了这条加法旁路，
        梯度就有了一条「高速公路」直达每一层。这是 2015 年 ResNet 带来的洞见，Transformer 全盘继承。
      </p>
      <Callout variant="info" title="把残差流想成一块「公共白板」">
        <p>
          一个特别好用的心智模型：残差流是一块从输入贯穿到输出的<strong>公共白板</strong>，初始写着 token 的 embedding。
          每一层不是「擦掉重写」，而是「在白板上<strong>追加</strong>一点笔记」（加上自己算的增量）。浅层追加表层信息（这是个名词、
          后面跟着标点），深层追加抽象信息（这其实在说一件因果关系）。最后读白板的最终状态，投影成 logits。
          这个视角能解释后面很多现象，比如为什么模型能「层层精炼」语义、为什么有些信息在中间层就已经「写好」了。
        </p>
      </Callout>
      ...
      <h3>注意力管「横」，FFN 管「纵」：一个关键直觉</h3>
      <p>
        再把这两个子层的分工掰细一点，因为它是理解整个 Transformer 的钥匙。注意力子层是<strong>跨位置</strong>的——
        token A 的输出会用到 token B、C、D 的信息，它在序列的「横向」上搬运、混合信息。FFN 子层则是<strong>逐位置</strong>的——
        它对每个 token 各算各的，位置之间完全不交流，只在单个向量的「纵向」上做加工。
      </p>
      <p>
        于是一层 block 的完整动作是：先用注意力「广纳信息」（横向汇聚上下文），再用 FFN「独立消化」（纵向加工提炼）。
        信息的「流动」和「加工」被干净地拆成两步交替进行。理解这个分工，你就明白为什么少了任何一个都不行：只有注意力，
        信息搬来搬去却没人加工；只有 FFN，每个 token 关起门来算、永远看不到上下文。
...
# W2: 4*d_model -> d_model（降维）`}
      ...
      <p>
        为什么非要「先升维再降维」绕这么一圈，不直接 d_model 到 d_model？因为<strong>非线性需要空间施展</strong>。
        在更高维（4 倍宽）的空间里过激活函数，模型能表达更复杂、更细的特征组合，再压回原宽度。这就像把一团面先擀开（升维）、
        切出花样（激活），再叠回去（降维）——中间那块更大的操作台，是表达力的来源。
      </p>
      <p>
        激活函数本身则解决了一个根本问题：<strong>没有它，再多层也等于一层</strong>。因为若全是线性变换（矩阵乘），
        无论叠多少层，整体仍是一个线性变换（矩阵的乘积还是矩阵），表达不了「异或」这种最基础的非线性关系。
        激活函数引入的「弯折」，才让深度真正带来能力。这也是为什么 FFN 这个看似平平无奇的子层，是模型不可或缺的一半。
      </p>
      ...
      <h3>LayerNorm 到底在做什么，为什么 RMSNorm 能省</h3>
      <p>
        LayerNorm 的动作是：对一个向量，先减去它自己的均值、再除以它自己的标准差，把它「拉回」到均值 0、方差 1 的标准形态，
        最后再乘一个可学的缩放、加一个可学的偏移。目的是让每一层的输入<strong>数值尺度稳定</strong>——否则几十层叠下来，
        数值要么越滚越大（爆炸）、要么越缩越小（消失），训练根本收不住。
      </p>
      <p>
        现代很多模型改用 <em>RMSNorm</em>（均方根归一化）：它<strong>省掉了「减均值」和「加偏移」两步</strong>，
        只用向量的均方根来缩放。实践发现这两步对效果影响不大，但去掉后计算更省、更快。这是大模型工程里典型的「砍掉收益不明显的零件、
        换取速度」的思路——知道它和 LayerNorm 同属一类、只是更精简，就够用了。
      </p>
      ...
      <h2>宽 vs 深，与 scaling law 的直觉</h2>
      <p>
        既然参数量约等于 <code>12·d²·层数</code>，做大模型有两条路：加<strong>宽</strong>（提高 d_model）或加<strong>深</strong>（增加层数）。
        宽度按平方涨参数、深度按线性涨参数。实践中两者要<strong>配比着加</strong>——只加深不加宽，深层会难训且收益递减；
        只加宽不加深，又缺少层级抽象的「步数」。各家大模型的 d_model 和层数大致维持着一个经验比例，不是随便定的。
      </p>
      <p>
        更宏观的规律是 <em>scaling law</em>（缩放定律）：模型的效果（用 loss 衡量）随<strong>参数量、数据量、算力</strong>的增长，
        呈现可预测的幂律下降。它的实践意义极大——你可以用小规模实验「外推」出大规模会有多好，从而在烧大钱训练前就规划好
        「该用多大模型、配多少数据」。著名的 Chinchilla 结论是：很多模型其实「参数过大、数据不足」，同样算力下把模型做小点、
        数据喂多点反而更优。这解释了为什么后来的模型不再一味比参数量，而是更讲究「参数和数据的配比」。
      </p>
      <Callout variant="info" title="MoE：把 FFN 拆成一堆专家">
        <p>
          既然 FFN 占了大头参数，一个自然的优化是<em>混合专家</em>（Mixture of Experts, MoE）：把每层那个大 FFN 换成
          <strong>很多个小 FFN（专家）</strong>，每个 token 只<strong>路由</strong>到其中少数几个去算。这样「总参数量」可以做得极大
          （知识容量大），但「单次推理实际激活的参数」却很少（算得快）。所以看到「几千亿参数」的 MoE 模型别被吓到——
          要看它的<strong>激活参数量</strong>，那才反映单次推理的真实算力和成本。这也呼应了下面警告里那条「总参数会误导」。
        </p>
      </Callout>
