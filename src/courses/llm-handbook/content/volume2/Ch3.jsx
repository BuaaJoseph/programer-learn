...
export default function Ch3() {
  return (
    <>
      ...
      <p>
        换个更底层的说法：模型单次前向传播的「深度」是固定的（第 1 卷第 5 章，层数有限），这决定了它一步能做的<strong>串行推理步数</strong>有上限。
        而 CoT 把推理摊到「生成很多个 token」上，每个 token 又是一次完整的前向传播——于是有效的串行计算步数，从「固定层数」扩展到了「层数 × 推理 token 数」。
        这是 CoT 能突破单步算力天花板的<strong>结构性原因</strong>，不是玄学。
      </p>

      <Callout variant="info" title="一个类比：心算 vs 列竖式">
        <p>
          让你直接报出「873 × 49」的得数，你多半要么算很久要么报错；但给你纸笔列竖式，一步步来，你几乎不会错。
          模型也一样：直接要答案，等于逼它「心算」，单步算力不够就蒙；让它写出每一步（CoT），等于给它纸笔。
        </p>
        <p>
          这个类比还能解释一个现象：<strong>越难的题，CoT 的增益越大</strong>。简单题模型「心算」就够，写不写过程差别不大；
          难题、多步推理题，CoT 才显出威力。所以别无脑给所有任务都加「一步步想」——对简单任务它只是白白增加 token 和延迟。
        </p>
      </Callout>
      ...
      <p>
        三者的关系是<strong>逐级加码</strong>：zero-shot CoT 最省（加一句话），few-shot CoT 用示例换更稳的格式和更高的正确率（多花点示例 token），
        self-consistency 用「多跑几遍 + 投票」换更高的可靠性（成本翻 n 倍）。按任务的重要性和预算往上加：日常用 zero-shot，
        格式要求高用 few-shot，关键且容错率低的题才上 self-consistency。
      </p>

      <h3>三种用法的进阶变体</h3>
      <p>
        在三种基本用法之上，还有几个值得知道的延伸：
      </p>
      <ul>
        <li>
          <strong>least-to-most</strong>（由简到繁）：先让模型把大问题<strong>拆成若干子问题</strong>，再逐个解决、把前一个的答案喂给后一个。
          适合那种「必须分解才能下手」的复杂题，比单纯「一步步想」更有结构。
        </li>
        <li>
          <strong>plan-and-solve</strong>（先规划再执行）：先让它列出解题计划，再按计划执行。避免它一头扎进去算错方向。
        </li>
        <li>
          <strong>给推理一个固定模板</strong>：在 few-shot 示例里把推理写成固定步骤（「第一步识别已知量；第二步列关系式；第三步代入计算」），
          模型会模仿这个结构，输出更可控、更易校验。
        </li>
      </ul>
      <p>
        共同的内核还是那一句：<strong>把一次性的难题，拆成模型能逐步消化的小步，并让中间结果落到文本里</strong>。
        这些变体只是「怎么拆、按什么顺序拆」的不同策略。
      ...
      <h2>CoT 的成本与「什么时候别用」</h2>
      <p>
        CoT 不是免费的午餐，它的代价很实在：
      </p>
      <ul>
        <li><strong>更费 token</strong>：推理过程本身要生成大量 token，输出长度可能翻几倍，直接推高费用。</li>
        <li><strong>更慢</strong>：生成是串行的（第 1 卷），推理越长，首字到完成的时间越久，对交互式场景体验有损。</li>
        <li><strong>可能「想多了」</strong>：简单题强行让它长篇推理，偶尔反而绕进死胡同、把对的想成错的。</li>
      </ul>
      <p>
        所以要按场景权衡：<strong>难推理任务（数学、逻辑、多步规划）值得开 CoT；简单分类、抽取、格式转换则不必</strong>。
        还有一个折中——让它推理但<strong>不输出</strong>：内部想完，最终只返回结论（或把推理放进一个会被丢弃的字段）。
        这样既享受推理带来的正确率，又不让冗长的过程污染最终交付给用户的内容。
      </p>
      <Callout variant="info" title="CoT 与结构化输出会「打架」">
        <p>
          一个高频的工程冲突：你既想让模型「先推理再答」（CoT），又想让它「只输出干净的 JSON」（结构化输出，第 4 章）。
          但推理过程是自然语言，硬塞进严格 JSON 模式要么推理被压制、要么 JSON 被推理文字污染。
        </p>
        <p>
          常见解法是在 schema 里专门留一个 <code>reasoning</code> 字段让它写推理，<code>answer</code> 字段放结论，
          解析时只取 <code>answer</code>。或者分两次调用：第一次自由推理，第二次把结论整理成结构化。
          知道这个冲突的存在，能帮你避开「为什么开了 JSON mode 模型就变笨了」的坑。
        </p>
      </Callout>
      ...
      <h2>从 CoT 到推理模型</h2>
      <p>
        把 CoT 这个思路推到极致，就得到了近年的<strong>推理模型</strong>（reasoning models）。区别在于：普通模型的 CoT 要你在 prompt 里手动引导
        （「一步步想」），而推理模型是<strong>被专门训练成「回答前先生成一大段思考」</strong>——这段思考往往很长、会自我检查、会回头纠错、
        会尝试多条路径，然后才给出最终答案。本质上，它把「写草稿」这件事内化成了默认行为。
      </p>
      <p>
        这带来一个工程上的新旋钮：<strong>思考预算</strong>（thinking budget / reasoning effort）。给的思考 token 越多，难题上越准，但也越慢越贵。
        所以用推理模型时，要按任务难度调这个预算——简单任务调低甚至关掉，难的数学/代码/规划任务才放开。
        理解了「CoT = 用 token 换计算」，你就明白推理模型为什么又强又贵：它就是在大量地用思考 token 买正确率。
      </p>
      <Callout variant="info" title="别让推理模型「再想一遍」">
        <p>
          一个常见误用：对已经内置思考的推理模型，还在 prompt 里堆「请一步步详细推理、先列出所有步骤……」。这往往多余甚至有害——
          它已经会自己想了，你的引导可能干扰它的内部推理节奏。对推理模型，prompt 反而要<strong>更简洁</strong>，把题目说清楚就行，
          把「怎么想」交给它。这和普通模型「要手动加 CoT」的直觉正好相反，别搞混。
        </p>
      </Callout>
