...
export default function Ch1() {
  return (
    <>
      ...
      <h2>prompt 工程在整个工具箱里的位置</h2>
      <p>
        在动手之前先校准期待：prompt 不是万能的。让模型表现更好，大致有三个层级的手段，成本和威力递增——
      </p>
      <ul>
        <li><strong>prompt 工程</strong>：改输入。零成本、立刻见效、随时可调，是绝大多数问题的<strong>第一选择</strong>，也是本卷主题。</li>
        <li><strong>RAG / 工具</strong>：给模型补充它没有的知识或能力（查资料、算数、调 API）。解决「参数里没有」的问题（第 5 卷）。</li>
        <li><strong>微调</strong>：改模型权重本身。成本高、周期长，只在 prompt 和 RAG 都搞不定特定风格/格式时才考虑。</li>
      </ul>
      <p>
        实战铁律：<strong>能用 prompt 解决的，绝不先上微调</strong>。很多人一遇到效果不好就想着「是不是要训练个专属模型」，
        其实九成问题靠把 prompt 写对、把对的资料检索进来就解决了。先穷尽便宜手段，再动昂贵手段。
      </p>
      ...
      <Example title="六要素逐条补齐，输出怎么变稳">
        <p>以「帮我写产品周报」为例，看每补一个要素，模型的不确定性怎么收窄：</p>
        <table>
          <thead>
            <tr><th>加上的要素</th><th>prompt 变成</th><th>消除了什么不确定</th></tr>
          </thead>
          <tbody>
            <tr><td>（只有任务）</td><td>帮我写产品周报</td><td>什么都没定，模型瞎猜格式和内容</td></tr>
            <tr><td>+ 上下文</td><td>…基于这周的 3 个进展：A、B、C</td><td>有了素材，不再凭空编</td></tr>
            <tr><td>+ 输出格式</td><td>…分「已完成/进行中/风险」三段，每段用要点</td><td>结构固定下来</td></tr>
            <tr><td>+ 角色</td><td>…以产品负责人口吻，对上级汇报</td><td>语气和详略锚定</td></tr>
            <tr><td>+ 约束</td><td>…总长不超过 200 字，不写套话</td><td>篇幅和风格收死</td></tr>
          </tbody>
        </table>
        <p>
          每一行都在「缩小模型的选择空间」。补到这一步，十次生成出来的周报会高度相似、且都能直接用——这就是「信息充分让分布变尖」的实感。
        </p>
      </Example>
      ...
      <h2>zero-shot 还是 few-shot：怎么选</h2>
      <p>
        不给示例叫 <em>zero-shot</em>（只靠指令），给示例叫 <em>few-shot</em>。该用哪个？一个简单的判断：
      </p>
      <ul>
        <li>
          <strong>任务常见、能用一句话说清</strong>（翻译、总结、改写）：zero-shot 往往就够，现代模型对这类任务训练充分。先试 zero-shot，省 token。
        </li>
        <li>
          <strong>输出格式特殊、或有微妙的风格/粒度要求</strong>（按你独有的模板填表、特定语气、边界判断）：上 few-shot，一个示例顶一段描述。
        </li>
        <li>
          <strong>任务罕见、模型容易理解偏</strong>：few-shot 几乎必须，用示例把「我到底要什么」演示给它看。
        </li>
      </ul>
      <p>
        实战顺序通常是：先 zero-shot 试一版，不行就加 1 个示例，还不行再加到 2 到 5 个，并补上边界 case。
        别一上来就堆十个示例——既贵，又可能因为示例间细微不一致反而干扰模型。
      </p>
