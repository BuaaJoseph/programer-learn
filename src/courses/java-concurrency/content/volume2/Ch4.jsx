    ...
    <>
      ...
      <Summary
        points={[
          ...
          'ReentrantLock 必须手动 unlock 且要放在 finally，否则异常时锁不释放会死锁。',
          'Condition 的 await/signal 对应 wait/notify，但一把锁可建多个 Condition，实现分组精准唤醒。',
          'ReentrantReadWriteLock：读读并发、读写互斥、写写互斥，适合读多写少；支持锁降级，不允许锁升级。',
          'StampedLock（JDK8）提供乐观读，读极多写极少时更快，但不可重入、不支持 Condition。',
