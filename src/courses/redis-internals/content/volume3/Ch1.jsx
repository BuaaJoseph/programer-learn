    ...
    <>
      ...
      <KeyIdea title="两种方案怎么选：优缺点对比">
        ...
        <p>
          ...
          适合数据量大、攻击 key 高度随机的场景。实战中两者常<strong>组合使用</strong>：布隆挡掉大部分，空值兜底剩余。
        </p>
      </KeyIdea>
      ...
      <p>
        ...
        请求全打到 DB），再说<strong>危害</strong>（恶意攻击或异常请求直接压垮数据库），然后给出<strong>三层防护</strong>：
        入口处参数校验拦掉明显非法请求，缓存空值兜底，布隆过滤器前置拦截大流量；最后<strong>对比两种缓存方案的取舍</strong>，
        说明实际会组合使用。能把「为什么空值要设短过期」「布隆为什么不会误杀存在的 key」讲透，就明显高出一档。
      </p>

      <Practice title="实现缓存空值 + 布隆过滤拦截">
        <p>
          下面是缓存空值的核心逻辑：数据库查不到时，把一个占位符 <code>__NULL__</code> 写进缓存并设短过期，
          下次同样的查询就能被缓存挡下。
        </p>
        <CodeBlock lang="python" title="cache_null.py" code={nullCacheCode} />
        <p>
          再加上布隆过滤器做前置拦截，把「一定不存在」的请求挡在最外层，连缓存都不用碰：
        </p>
      ...
      <Summary
        points={[
          ...
          '防护一是缓存空值：数据库查空时把空也缓存起来，并设较短过期时间避免占内存、便于后续替换。',
          '防护二是布隆过滤器：预存合法 key，请求先问它，说「不存在」就一定不存在，直接拒掉。',
          '入口处的参数合法性校验是成本最低的第一道防线，明显非法的请求该在最前面拒绝。',
          '空值方案简单但占内存、有短暂一致性损失；布隆省内存能扛海量但有误判、需维护 key 集合，实战常组合使用。',
        ]}
      />
    </>
  )
