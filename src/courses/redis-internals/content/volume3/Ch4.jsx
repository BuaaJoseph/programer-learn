...
const mqRetryCode = `# 删缓存失败的兜底：投到 MQ 由消费者反复重试，直到成功
def update_price_with_retry(product_id, new_price):
    db.update_price(product_id, new_price)   # 1. 先更库
    key = "product:" + str(product_id)
    try:
        redis.delete(key)                    # 2. 尝试删缓存
    except RedisError:
        mq.send("cache:invalidate", key)     # 3. 删失败，投递重试消息

# 消费者：拿到 key 后反复删，失败则重新入队(带退避)，直到删成功
def on_invalidate_message(key):
    redis.delete(key)                        # 幂等：删多少次都没问题`
...
export default function Ch4() {
  return (
    <>
      ...
      <h2>四种更新顺序，两两对比</h2>
      <p>
        缓存更新无非是「更新 / 删除缓存」乘以「先动缓存 / 先动 DB」四种组合。先把「更新缓存」的两种排除掉
        （第一章已讲：会并发覆盖、且重算浪费），重点对比「删缓存」的两种顺序：
      </p>
      <ul>
        <li>
          <strong>先删缓存，再更 DB</strong>：删完缓存、还没更新 DB 的窗口里，读请求未命中会从 DB 读到<strong>旧值并回写缓存</strong>，
          随后 DB 才更新 —— 缓存留下旧值，长期不一致。这个窗口大、概率高。
        </li>
        <li>
          <strong>先更 DB，再删缓存</strong>（推荐）：也有极小概率不一致——读请求在「更 DB 前」读到旧值、在「删缓存后」才回写，
          但这要求读比写还慢且时序极其凑巧，实践概率远低于前者。
        </li>
      </ul>
      ...
      <CodeBlock lang="python" title="mq_retry_delete.py（删缓存失败投 MQ 重试）" code={mqRetryCode} />
      <p>
        binlog 订阅这条路在大型系统里尤其受青睐：它把「写库」和「失效缓存」彻底解耦，业务代码完全无感，
        无论数据是被哪个服务、哪条 SQL 改的，只要进了 binlog 就会触发删缓存，<strong>不会漏</strong>。
        代价是引入了 canal 这类中间件、有秒级延迟。
      </p>
      ...
      <p>
        ...
        的耗时（常取几百毫秒），确保把那条可能的脏数据覆盖掉。主从架构下延迟还要覆盖<strong>主从同步延迟</strong>，否则第二次删完、
        从库还没同步好，读从库仍可能回写旧值。
      </p>
      ...
      <KeyIdea title="强一致代价大，多数业务接受最终一致">
        <p>
          ...
          靠「删缓存 + 失败重试 + 延迟双删 / binlog 订阅 + 过期兜底」把窗口压到足够小、把最终结果对齐。
      ...
      <table>
        <thead>
          <tr><th>策略</th><th>一致性</th><th>复杂度</th><th>适合</th></tr>
        </thead>
        <tbody>
          <tr><td>先更 DB 再删缓存 + 过期兜底</td><td>最终一致(窗口极小)</td><td>低</td><td>绝大多数业务</td></tr>
          <tr><td>延迟双删</td><td>最终一致(更稳)</td><td>中</td><td>读写并发高</td></tr>
          <tr><td>binlog 订阅(canal)</td><td>最终一致(不漏)</td><td>高(需中间件)</td><td>多写入源、彻底解耦</td></tr>
          <tr><td>分布式锁串行化</td><td>接近强一致</td><td>高(性能差)</td><td>极少数强一致场景</td></tr>
        </tbody>
      </table>
      ...
      <Summary
        points={[
          ...
          '删缓存失败的兜底：用消息队列重试，或订阅 binlog 由 canal 异步删缓存(不漏、彻底解耦)，保证最终一致。',
          '延迟双删：先删缓存→更新 DB→延迟再删一次，清掉窗口期内被旧值回写的脏缓存；主从下延迟要覆盖同步延迟。',
          '强一致代价大，多数业务接受最终一致，再以「关键操作以数据库为准」的校验和过期时间兜底。',
        ]}
      />
    </>
  )
