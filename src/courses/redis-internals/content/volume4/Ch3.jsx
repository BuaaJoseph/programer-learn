...
const eventCmd = `# 故障转移时哨兵广播的关键事件(可用 redis-cli psubscribe 订阅观察)
+sdown master mymaster 127.0.0.1 6379      # 主观下线：某哨兵 ping 不通
+odown master mymaster 127.0.0.1 6379 #quorum 2/2  # 客观下线：达到 quorum
+vote-for-leader <runid> <epoch>           # Raft 投票选 Leader
+switch-master mymaster 127.0.0.1 6379 127.0.0.1 6381  # 主库地址切换完成
+slave-reconf-done slave ...               # 其余从库改指向新主完成

# 客户端无需关心细节：支持哨兵的库会自动订阅 +switch-master 并重连新主`
  ...
  return (
    <>
      ...
      <p>
        ...
        值得一提的是，你只需在配置里写主库地址，哨兵会通过 <code>INFO</code> 命令<strong>自动发现</strong>它的所有从库，
        也能通过哨兵之间的发布订阅<strong>互相发现</strong>，无需手动配置全部拓扑。
      </p>
      ...
      <Callout variant="warn" title="quorum 不是用来选主的">
        <p>
          ...
          还有个细节：选 Leader 需要拿到<strong>「半数以上的全部哨兵」</strong>的票（majority），这个数和 quorum 是两回事——
          所以 quorum 设得比半数小也没用，最终选主仍要过半数哨兵存活。
        </p>
      </Callout>
      ...
      <p>
        ...
        要等下一轮（epoch +1）重选。选出 Leader 后，由它一个人主导整个故障转移，避免多个哨兵同时操作造成混乱。
      </p>
      ...
      <ul>
        <li>
          <strong>选新主</strong>：从存活的从库里挑一个最优的，挑选规则按优先级依次是：剔除掉线/延迟过大的从库 →
          <code>replica-priority</code> 数值小的优先（0 表示永不当主）→ 复制 <code>offset</code> 最大即<strong>数据最新</strong>的 →
          <code>runid</code> 最小的。对选中的从库执行 <code>replicaof no one</code> 升为新主。
        </li>
        <li>
          <strong>其余从库改指向</strong>：让剩下的从库执行 <code>replicaof 新主</code>，重新挂到新主下面同步
          （<code>parallel-syncs</code> 控制同时同步的数量，避免一起全量把新主拖垮）。
        </li>
        <li>
          <strong>通知客户端</strong>：通过发布订阅广播 <code>+switch-master</code> 事件，并把<strong>旧主</strong>降级——它恢复后会作为新主的从库回归。
        </li>
      </ul>
      <CodeBlock lang="text" title="故障转移期间的哨兵事件" code={eventCmd} />
      ...
      <Callout variant="warn" title="脑裂：哨兵也救不了的数据丢失">
        <p>
          网络分区时可能<strong>脑裂</strong>：旧主和部分客户端被分到一个区，哨兵在另一个区把某从库提成了新主。
          这期间旧主仍在接收写（客户端没切过去），等网络恢复，旧主被降级为从库、清空数据去同步新主，
          于是<strong>分区期间写到旧主的数据全丢了</strong>。缓解办法就是上一章的 <code>min-replicas-to-write</code> +
          <code>min-replicas-max-lag</code>：旧主发现连不上足够从库时主动<strong>拒绝写</strong>，把损失降到最低。这是高频面试追问。
        </p>
      </Callout>
      ...
      <p>
        ...
        哨兵进程也应<strong>分散部署在不同机器</strong>，否则一台机器挂掉带走多个哨兵就失去了冗余意义。
      </p>
      ...
      <KeyIdea title="客户端如何拿到新主地址">
        <p>
          ...
          一旦发生故障转移，客户端收到 <code>+switch-master</code> 通知后<strong>自动重连新主</strong>，业务代码无需改动。这正是哨兵方案对应用透明的根本原因。
        </p>
      </KeyIdea>
      ...
      <p>
        ...
        单哨兵 ping 超时是主观下线，多个哨兵达到 quorum 共识是客观下线；客观下线后用 Raft 选 Leader（要过半数哨兵票）执行切换
        （选数据最新即 offset 最大的从库升主、其余改指向、广播通知）；哨兵自己要奇数台集群且分散部署避免误判和脑裂；客户端连哨兵而非主库，切换后自动重连新主。
        能主动补一句「脑裂会丢数据、靠 min-replicas 缓解」就明显高出一档。
      </p>
      ...
      <Summary
        points={[
          '哨兵是独立进程，负责监控、通知、自动故障转移，并能自动发现从库与彼此，解决主从复制无法自动切主的问题。',
          '主观下线 SDOWN 是单个哨兵 ping 超时的单方判断；客观下线 ODOWN 需达到 quorum 个哨兵共识才成立。',
          'quorum 只用于判定客观下线，不是选主；客观下线后哨兵用 Raft 选出唯一 Leader(需过半数哨兵票)来执行切换。',
          '故障转移三步：选 offset 最大(数据最新)的从库升为新主、其余从库改指向新主、广播通知并把旧主降为从库。',
          '哨兵要部署奇数个、分散在不同机器，便于 Raft 过半选举、互相印证避免误判与脑裂。',
          '脑裂会丢分区期间写入旧主的数据，靠 min-replicas-to-write/max-lag 让旧主拒绝写来缓解。',
          '客户端配置哨兵地址而非主库地址，订阅 +switch-master 事件，故障后自动重连新主、对应用透明。',
        ]}
      />
    </>
  )
