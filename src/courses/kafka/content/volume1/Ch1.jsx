    ...
    <>
      ...
      <Summary
        points={[
          ...
          '三大典型场景：日志收集、流处理、解耦削峰，都看中它的高吞吐与可回放。',
          '它不擅长复杂业务路由、消息优先级、单条级 ack 重试，这类需求 RabbitMQ 更合适。',
          '高吞吐来自四件事叠加：分区并行、顺序写、零拷贝、批量加压缩，后面三章逐一展开。',
          '动手：用 kafka-topics 建 topic，用 console 生产者消费者收发消息，--from-beginning 可重读历史。',
        ]}
      />
    </>
  )
