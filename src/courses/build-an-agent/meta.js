export const meta = {
  ...
  pricing: 'free',
  description:
    '跟着课程，从一个空文件夹开始，用 Node + TypeScript 亲手写出一个真正能用、能像 Claude Code 一样 npm 安装的编码 Agent —— forge。每一章对应仓库里的一段真实代码：从主循环、工具契约、读写工具，到 CLI 流式渲染、权限与人在回路、上下文工程与自动压缩、子代理、Provider 抽象与 MCP，最后打包发布到 npm。学完你不只是「会用 Agent」，而是「能造 Agent」。',
