...
# 现在开始
请以面试官身份开场：第一行写 [[STAGE:intro]]，然后先简短自我介绍，再邀请候选人做自我介绍。直接输出你要说的话，不要输出任何解释性的旁白或括号说明。`
}

// 解析回复开头的阶段标记，返回 { stage, clean }（clean 已去掉标记）。
// 同时容忍标记残缺（流式中尚未闭合）的情况由调用方处理。
export function parseStageTag(raw) {
  const m = String(raw).match(/^\s*\[\[STAGE:\s*([a-zA-Z]+)\s*\]\]\s*\n?/)
  if (m) return { stage: m[1].toLowerCase(), clean: raw.slice(m[0].length) }
  return { stage: null, clean: raw }
}

export const STAGE_INDEX = { intro: 0, project: 1, tech: 2, llm: 3, coding: 4 }
